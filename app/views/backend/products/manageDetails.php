				...
				if(isset($type) && $type == "add" || $type == "edit" || $type == "view")
				{
						...
						<div class="header-lines">
							...
							<div class="row mb-3">
								...
								<div class="col-md-6 text-right">
									...
									<a href="<?php echo base_url(); ?>products/manageDetails" class="btn btn-default btn-sm">Close</a>
								</div>
							</div>
							<!-- Buttons end here -->
									...
									<div class="row">
										<div class="col-md-6">
											...
											<div class="row">
												<div class="col-md-4">
													<div class="form-group text-right">
														<label class="col-form-label header_details_image"><span class="text-danger">*</span> Image</label>
													</div>				
												</div>
												<div class="col-md-4">
													<div class="form-group">
														<input type="file" name="header_details_image" id="header_details_image" onchange='validateImage(this)' accept='.png, .gif, .jpg, .jpeg, .bmp' class="form-control singleImage">
														<?php 
															if( ($type == "edit" || $type == "view") && isset($id))
															{
																$url = "uploads/products/details/header_img/".$id.".png";
																if(file_exists($url))
																{
																	?><br>
										...
										<table class="table table-bordered table-hover line_items" id="line_items">
											<thead>
												<tr>
													...
													<th class="tab-md-130"><span class="text-danger">*</span> Line Decsription</th>
														...
														if( count($lineData) > 0)
														{
															...
															foreach($lineData as $lineItems)
															{
																...
																<tr class="remove_tr tabRow<?php echo $counter;?>">
																	...
																	<?php /*
																	<td class="tab-md-85">
																		<input type="file" name="details_image[]" id="details_image" class="form-control singleImage" onchange='validateImage(this)'>
																		<?php 
																			if( ($type == "edit" || $type == "view") && isset($lineItems["line_id"]))
																			{
																				$url = "uploads/products/details/".$lineItems["line_id"].".png";
																				if(file_exists($url))
																				{
																					?>
					...
					<script>
						...
						function updateStatus(checkbox, id) 
						{
							...
							$.ajax({
								type: "GET",
								url: "<?php echo base_url().'products/ajaxAvailableDetails/status/';?>" + id + "/" + status,
								data: {}
							}).done(function(msg) 
							{
								toastr.success(msg);
						...
						function saveBtn(val) {
							...
							var title 					= $("#title").val();
							var header_details_image 	= $("#header_details_image").val();
							...
							if (product_id && title ) {
								...
								$("table.line_items").find('tr').each(function () {
									...
									// var detail_image    	= row.find('input[name^="detail_image[]"]').val();
								...
								if (type === 'add' && !header_details_image) {
									$(".header_details_image").addClass('errorClass');
								} else {
									$(".header_details_image").removeClass('errorClass');
								}
								return false;
							}
						}
						...
						function addLines(val) {
							if (val == 1) {
								...
								if (product_id && title && (type !== 'add' || header_details_image)) 
								{
									$(".product_id").removeClass('errorClass');
									$(".title").removeClass('errorClass');
									addSOLines();
							...
							$.ajax({
								...
								success: function(d)
								{
									...
									if(flag == 0)
									{
										...
										// cols += "<td class='tab-md-150'>" 
										// 	+"<input type='file' class='form-control' name='details_image[]' id='details_image"+ counter +"' value='' onchange='validateImage(this)'></input>"
										// +"</td>";
				...
				{ 
					...
					<div class="row">
						...
						<div class="col-md-6 float-right text-right">
							<a href="<?php echo base_url(); ?>products/manageDetails/add" class="btn btn-info btn-sm">
								Create Details
							</a>
							<?php /*
					...
					<form action="" class="form-validate-jquery" method="get">
						...
						<div class="row">
							<div class="col-md-12 text-right float-right">
								...
								<a href="<?php echo base_url(); ?>products/manageDetails" title="Clear" class="btn btn-default">Clear</a>
							</div>
						</div>
														...
														<td  class='text-center' style="width:90px;">
															<div class="dropdown" style="width:90px;">
																...
																<ul class="dropdown-menu dropdown-menu-right dropdown-menu-new">
																			...
																			<li>
																				<a href="<?php echo base_url(); ?>products/manageDetails/edit/<?php echo $row['header_id'];?>">
																					<i class="fa fa-pencil"></i> Edit
																				</a>
																			</li>
																			
																			<li>
																				<a href="<?php echo base_url(); ?>products/manageDetails/view/<?php echo $row['header_id'];?>">
																			...
																			<li>
																				<?php 
																					...
																					{
																						?>
																						<a class="unblock" href="<?php echo base_url(); ?>products/manageDetails/status/<?php echo $row['header_id'];?>/N" title="Inactive">
																					...
																					{  ?>
																						<a class="block" href="<?php echo base_url(); ?>products/manageDetails/status/<?php echo $row['header_id'];?>/Y" title="Active">
