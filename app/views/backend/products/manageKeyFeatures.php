				...
				if(isset($type) && $type == "add" || $type == "edit" || $type == "view")
				{
						...
						<div class="header-lines">
							...
							<div class="row mb-3">
								...
								<div class="col-md-6 text-right">
									...
									<a href="<?php echo base_url(); ?>products/manageKeyFeatures" class="btn btn-default btn-sm">Close</a>
								</div>
							</div>
							<!-- Buttons end here -->
									...
									<div class="row">
										<div class="col-md-6">
											<div class="row">
												<div class="col-md-4">
													<div class="form-group text-right">
														<label class="col-form-label product_id"><span class="text-danger">*</span> Product Name</label>
													</div>				
												</div>
												<div class="col-md-5">
													<div class="form-group">
														<div class="input-wrapper">
															<input type="text" name="product_name" autocomplete="off" id="product_name" value="<?php echo isset($headerData[0]['product_name']) ? $headerData[0]['product_name'] : NULL; ?>" placeholder="Product Name" class="form-control">
															<input type="hidden" name="product_id" autocomplete="off" id="product_id" value="<?php echo isset($headerData[0]['product_id']) ? $headerData[0]['product_id'] : NULL; ?>" >
															<div id="ProductNameList"></div><!-- Clear icon start -->
																<?php 
																	if(isset($headerData[0]["product_id"]) && !empty($headerData[0]["product_id"]))
																	{
											...
											<div class="row">
												...
												<div class="col-md-4">
													<div class="form-group">
														<textarea name="title" id="title" cols="" rows="1" placeholder="Title" class="form-control"><?php echo isset($headerData[0]['title']) ? $headerData[0]['title'] : NULL;?></textarea>
													</div>				
												</div>
											</div>
										</div>
										...
										<table class="table table-bordered table-hover line_items" id="line_items">
											<thead>
												<tr>
													...
													<th class="tab-md-130"><span class="text-danger">*</span> Line Desription</th>
													<th class="tab-md-130"><span class="text-danger">*</span> Detail Description</th>
														...
														if( count($lineData) > 0)
														{
															...
															foreach($lineData as $lineItems)
															{
																...
																<tr class="remove_tr tabRow<?php echo $counter;?>">
																	...
																	<td class="tab-md-85">
																		<textarea class="form-control" name="detail_description[]" id="detail_description<?php echo $counter;?>"><?php echo isset($lineItems["detail_description"]) ? $lineItems["detail_description"] : "";?></textarea>
																	</td>

																	<td class="tab-md-85">
																		<input type="file" name="key_features_image[]" id="key_features_image" class="form-control singleImage" onchange='validateImage(this)' accept='.png, .gif, .jpg, .jpeg, .bmp'>
																		<?php 
																			if( ($type == "edit" || $type == "view") && isset($lineItems["line_id"]))
																			{
																				$url = "uploads/products/key_features/".$lineItems["line_id"].".png";
																				if(file_exists($url))
																				{
																					?>
																					<?php /*
					...
					<script>
						...
						function updateStatus(checkbox, id) 
						{
							...
							$.ajax({
								type: "GET",
								url: "<?php echo base_url().'products/ajaxAvailableKeyFeatures/status/';?>" + id + "/" + status,
								data: {}
							}).done(function(msg) 
							{
								toastr.success(msg);
						...
						function saveBtn(val) {
							...
							if (product_id && title ) {
								...
								$("table.line_items").find('tr').each(function () {
									...
									var detail_description 	= row.find('textarea[name^="detail_description[]"]').val();
									var key_features_image	= row.find('input[name^="key_features_image[]"]').val();
									
									if( line_title === "" || line_description === "" || detail_description === "" || (key_features_image === "" && type !== 'edit'))
									{
										lineIncomplete = true;
										return false;
									}
								...
								if (!product_id) $(".product_id").addClass('errorClass');
								else $(".product_id").removeClass('errorClass');
							...
							$.ajax({
								...
								success: function(d)
								{
									...
									if(flag == 0)
									{
										...
										cols += "<td class='tab-md-150'>" 
											+"<textarea class='form-control' rows='1' name='detail_description[]' id='detail_description"+ counter +"' value=''></textarea>"
										+"</td>";

										cols += "<td class='tab-md-150'>" 
											+"<input type='file' class='form-control' name='key_features_image[]' id='key_features_image"+ counter +"' value='' onchange='validateImage(this)' accept='.png, .gif, .jpg, .jpeg, .bmp'></input>"
										+"</td>"
				...
				{ 
					...
					<div class="row">
						...
						<div class="col-md-6 float-right text-right">
							<a href="<?php echo base_url(); ?>products/manageKeyFeatures/add" class="btn btn-info btn-sm">
								Create Key Feature
							</a>
							<?php /*
					...
					<form action="" class="form-validate-jquery" method="get">
						<div class="row mt-3">
							<div class="col-md-4">
								<div class="row">
									<label class="col-form-label col-md-5">Product Name</label>
										...
										<div class="input-wrapper">
											...
											<div id="ProductNameList"></div><!-- Clear icon start -->
												<?php 
													...
													{
														$styleDisplay = "display:block;top:-18px ! important";
													}
													else{
														$styleDisplay = "display:none";
													}
						...
						<div class="row">
							<div class="col-md-12 text-right float-right">
								...
								<a href="<?php echo base_url(); ?>products/manageKeyFeatures" title="Clear" class="btn btn-default">Clear</a>
							</div>
						</div>
														...
														<td  class='text-center' style="width:90px;">
															<div class="dropdown" style="width:90px;">
																...
																<ul class="dropdown-menu dropdown-menu-right dropdown-menu-new">
																	<li>
																		<a href="<?php echo base_url(); ?>products/manageKeyFeatures/edit/<?php echo $row['header_id'];?>">
																			<i class="fa fa-pencil"></i> Edit
																		</a>
																	</li>
																	
																	<li>
																		<a href="<?php echo base_url(); ?>products/manageKeyFeatures/view/<?php echo $row['header_id'];?>">
																	...
																	<li>
																		<?php 
																			...
																			{
																				?>
																				<a class="unblock" href="<?php echo base_url(); ?>products/manageKeyFeatures/status/<?php echo $row['header_id'];?>/N" title="Inactive">
																			...
																			{  ?>
																				<a class="block" href="<?php echo base_url(); ?>products/manageKeyFeatures/status/<?php echo $row['header_id'];?>/Y" title="Active">
